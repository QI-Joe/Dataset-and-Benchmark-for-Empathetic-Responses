...
def multi_turn_chat_with_ppl_batched(
    ...
    new_tokens_tensor = gen_out[:, prompt_len:]
    ...
    seq_losses = (loss * mask).sum(dim=1) / mask.sum(dim=1)
...
def _modified_precision(hypothesis, reference, n):
    ...
    return numerator / denominator  # returns 0.0 when no n-gram matches

def compute_sentence_bleu(hypothesis_str, reference_str):
    # Use character-level tokenization for Chinese text (no spaces between words)
    hyp = list(hypothesis_str.strip())
    ref = list(reference_str.strip())
    if not hyp:
    ...
    bleu1 = bp * p[0]
    bleu2 = bp * math.exp(0.5  * math.log(p[0]) + 0.5  * math.log(p[1])) if p[0] > 0 and p[1] > 0 else 0.0
    bleu3 = bp * math.exp(1/3  * math.log(p[0]) + 1/3  * math.log(p[1]) + 1/3  * math.log(p[2])) if all(x > 0 for x in p[:3]) else 0.0
    bleu4 = bp * math.exp(0.25 * math.log(p[0]) + 0.25 * math.log(p[1]) + 0.25 * math.log(p[2]) + 0.25 * math.log(p[3])) if all(x > 0 for x in p) else 0.0
